...
USB=/media/jpg/JPG-BAKUP-2 #Remplacer DATA-JPG par le nom de votre disque dur
...
OPTIONS="-dirtoq --delete --include=.thunderbird --include=.mozilla --include=.ssh --include=.gnupg --include=.conky --exclude=.* --exclude=Téléchargements --exclude=Appimages --exclude=Arduino --exclude=Backup --exclude=Duplicates --exclude=Nextcloud --exclude=NextcloudGulliver --exclude=Vscodium --exclude=Warpinator --exclude=VirtualB* --exclude=vmware --log-file="$LOG"" 
