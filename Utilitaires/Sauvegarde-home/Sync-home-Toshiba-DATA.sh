...
OPTIONS="-dirtoq --delete --include=.thunderbird --include=.mozilla --include=.ssh --include=.gnupg --include=.gnome2/nautilus-scripts --include=.MonCoffreFort_encfs --include=.unison --include=.gnome2 --include=.conky --include=.sane --include=Hubic/.CoffrePerso_encfs --exclude=MEGA --exclude=.* --exclude=Vidéos --exclude=Musique --exclude=Images --exclude=Téléchargements --exclude=VirtualB* --exclude=vmware --log-file="$LOG"" 
